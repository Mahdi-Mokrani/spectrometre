...
Remarquons tout d'abord que pour mesurer un angle, nous effectuons toujours **deux visées**. De la même manière que, sur un banc d'optique, on détermine toujours une distance $d$ en soustrayant **deux abscisses** ($d=x_2-x_1$ ) ; sur un goniomètre, on détermine toujours un angle en soustrayant **deux azimuts**.
